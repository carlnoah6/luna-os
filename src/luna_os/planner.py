...
def normalize_step(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    if "step_type" in raw:
        result["step_type"] = raw["step_type"]
    if "branches" in raw:
        result["branches"] = raw["branches"]
...
def format_plan(plan: Plan) -> str:
    ...
    for s in plan.steps:
        ...
        if st == "done":
            ...
            # Show conditional node indicator and selected branch
            node_type = ""
            if s.step_type == "conditional":
                branch_info = f" → {s.selected_branch}" if s.selected_branch else ""
                node_type = f" 🔀{branch_info}"
            lines.append(f"  [done] {num}. {title}{node_type} {s.task_id or ''}{dur_str} -- {result}")
        elif st == "running":
            ...
            lines.append(f"  [skipped] {num}. {title} (branch not taken)")
        else:
            deps = s.depends_on or []
            dep_str = f" (after {','.join(str(d) for d in deps)})" if deps else ""
            # Show conditional node indicator for pending conditional nodes
            node_type = " 🔀" if s.step_type == "conditional" else ""
            lines.append(f"  [pending] {num}. {title}{node_type}{dep_str}")
...
"""
    ...
    def step_done(self, chat_id: str, step_num: int, result: str) -> dict[str, Any]:
        ...
        # Handle conditional node branch selection
        step = self.store.get_step(plan.id, step_num)
        if step and step.step_type == "conditional" and step.selected_branch:
            self._handle_branch_selection(plan.id, step)
    ...
    def replan(
        ...
        for i, ns in enumerate(new_steps):
            ...
            self.store.insert_step(
                ...
                model=ns.get("model"),
                step_type=ns.get("step_type", "normal"),
                branches=ns.get("branches"),
            )
    ...
    def check_contracts(self) -> dict[str, Any]:
        ...
        processed = process_events(
            ...
            on_replan=lambda plan, new_steps, append: self.replan(
                plan.chat_id, new_steps, append
            ),
        )
    ...
    def _handle_branch_selection(self, plan_id: str, step: Step) -> None:
        """Handle conditional node branch selection.
        
        When a conditional node completes with a branch selection:
        1. Find the selected branch definition
        2. Activate steps in the selected branch
        3. Skip (mark as skipped) steps in other branches
        4. Handle loops (if branch points back to earlier step)
        """
        if not step.branches or not step.selected_branch:
            return
        ...
        for branch in step.branches:
            if branch.get("name") == step.selected_branch:
                selected = branch
            else:
        ...
        if not selected:
            logger.warning(
                "Branch '%s' not found in step %d branches: %s",
                step.selected_branch, step.step_num, [b.get("name") for b in step.branches]
            )
            return
        
        # Get next steps for selected branch
        next_steps = selected.get("next_steps", [])
        
        # Collect all steps from other branches
        other_steps = set()
        for branch in other_branches:
            other_steps.update(branch.get("next_steps", []))
        
        # Skip steps that are only in other branches (not in selected branch)
        skip_steps = other_steps - set(next_steps)
        logger.info(
            "Branch selection: selected='%s', next_steps=%s, other_steps=%s, skip_steps=%s",
            step.selected_branch, next_steps, other_steps, skip_steps
        )
        for step_num in skip_steps:
            existing = self.store.get_step(plan_id, step_num)
            if existing and existing.status.value == "pending":
                self.store.update_step(plan_id, step_num, status="skipped")
                logger.info(
                    "Skipped step %d (not in selected branch '%s')",
                    step_num, step.selected_branch
                )
        
        # Handle loops: if next_steps contains a step number <= current step
        for next_step_num in next_steps:
            if next_step_num <= step.step_num:
                # This is a loop - increment execution count
                existing = self.store.get_step(plan_id, next_step_num)
                if existing:
                    new_count = existing.execution_count + 1
                    max_loops = 10  # Safety limit
                    if new_count > max_loops:
                        logger.warning(
                            "Loop limit reached for step %d (executed %d times)",
                            next_step_num, new_count
                        )
                        self.store.update_step(plan_id, next_step_num, status="cancelled")
                        self._notify(
                            step.plan_id,
                            f"⚠️ Loop limit reached for step {next_step_num} "
                            f"(executed {new_count} times). Stopping loop."
                        )
                    else:
                        # Reset the step to pending so it can run again
                        self.store.update_step(
                            plan_id, next_step_num,
                            status="pending",
                            task_id=None,
                            result=None,
                            started_at=None,
                        ...
                        logger.info(
                            "Loop: reset step %d to pending (execution #%d)",
                            next_step_num, new_count
                        )
                        
                        # Also reset all steps that depend on this step
                        # (they were skipped in previous iteration)
                        plan = self.store.get_plan(plan_id)
                        if plan:
                            for s in plan.steps:
                                if next_step_num in (s.depends_on or []):
                                    if s.status.value in ("skipped", "cancelled"):
                                        self.store.update_step(
                                            plan_id, s.step_num,
                                            status="pending",
                                            task_id=None,
                                        ...
                                        logger.info(
                                            "Loop: reset dependent step %d to pending",
                                            s.step_num
                                        )
